...
if __name__ == '__main__':
    ...
    def smooth(x,window_len=64,window='hanning'):
        x = numpy.array(x)
    ...
    def testdel(x, dotimes):
        ...
        return list(reversed(indexes)), tsmooth(times)
    ...
    ax.plot(*testget(sobj, els), label='get (after 16384 elements set)')
    ax.plot(*testin (sobj, els), label='in (after 16384 elements set)' )
    ax.plot(*testdel(sobj, els), label='del (after 16384 elements set)')
    pulverise_original()
    sobj = Shelve(FILENAME)
    ax.plot(*testgt2(sobj, els), label='get (after each element set)')
    pulverise_original()
